...
const Login = () => {
	...
	const submit = async() => {
		if(state.id.trim() === '') {tips("Invalid name"); updateStatus({error:1}); return;}
		if(state.password === '') {tips("Invalid password"); updateStatus({error:2}); return;}
		...
		if(response){
			switch(response['error']){
				...
				case 1: tips("Error: Cannot found admin"); updateStatus({error:1}); break; 
				case 2: tips("Error: Invalid password"); updateStatus({error:2}); break; 
						...
						<div className="row center m3 p3">
							<div className="col-lg-6 col-sm-12">
								<div className="pool-details-card ">
									...
									<div className={`text-center mt1 ${state.error !== 1 ?'hide':''}`} style={{color:'red'}} >Invalid Name</div>
									<div className="row pl3 pr3">
										<p>Password</p>
										<input type="password" className='w100' value={state.password} onChange={(e)=> {updateStatus({password:e.target.value})}}/>
									</div>
									<div className={`text-center mt1 ${state.error !== 2 ?'hide':''}`} style={{color:'red'}} >Invalid Password</div>
